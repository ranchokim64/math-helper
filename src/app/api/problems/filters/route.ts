...
const getCachedFilters = unstable_cache(
  async (selectedGrade: string | null, selectedSemester: string | null) => {
    // 전체 문제 수 확인
    ...
    const filters = {
      ...
      contentElements: contentElementsByArea,
  ...
  ['problem-filters'],
  {
    revalidate: false, // 만료X
    tags: ['problem-filters']
  }
)
...
export async function GET(request: NextRequest) {
  try {
    ...
    const selectedGrades = searchParams.get('grades') // 복수 학년 (쉼표로 구분)
    const selectedSemesters = searchParams.get('semesters') // 복수 학기 (쉼표로 구분)

    // 복수 조합 처리 (배치 요청)
    if (selectedGrades && selectedSemesters) {
      try {
        const gradesArray = selectedGrades.split(',')
        const semestersArray = selectedSemesters.split(',')

        // 모든 조합에 대해 캐시된 데이터 가져오기
        const combinations = gradesArray.flatMap(grade =>
          semestersArray.map(semester => ({ grade, semester }))
        )

        const results = await Promise.all(
          combinations.map(({ grade, semester }) => getCachedFilters(grade, semester))
        )
        ...
        const mergedFilters = {
          schools: results[0]?.schools || ["초등학교"],
          grades: results[0]?.grades || [],
          semesters: results[0]?.semesters || [],
          ...
          totalProblems: results[0]?.totalProblems || 0,
          lastUpdated: new Date().toISOString(),
          appliedFilters: {
            grades: gradesArray,
        ...
        return NextResponse.json(mergedFilters)
      } catch (error) {
        console.error("배치 필터 처리 오류:", error)
      }
    }

    // 단일 조합 처리 (기존 방식)
    try {
      const filters = await getCachedFilters(selectedGrade, selectedSemester)
