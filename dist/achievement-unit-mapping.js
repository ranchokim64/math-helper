...
exports.elementaryAchievementMapping = void 0;
exports.getUnitFromAchievement = getUnitFromAchievement;
exports.getAllUnits = getAllUnits;
exports.getUnitsBySchool = getUnitsBySchool;
exports.getUnitsByGrade = getUnitsByGrade;
// 초등학교 성취기준 매핑
exports.elementaryAchievementMapping = {
    // 1학년
...
// 통합 매핑 함수 (초등학교만 지원)
function getUnitFromAchievement(achievementCode) {
    // 성취기준 코드에서 대괄호 제거하고 정규화
    const cleanCode = achievementCode.replace(/[\[\]]/g, '').trim();
    const normalizedCode = `[${cleanCode}]`;
    // 초등학교 매핑 확인
    if (exports.elementaryAchievementMapping[normalizedCode]) {
        return exports.elementaryAchievementMapping[normalizedCode];
    }
    return null;
}
...
function getAllUnits() {
    const units = new Set();
    Object.values(exports.elementaryAchievementMapping).forEach(mapping => {
        units.add(mapping.unit);
    });
    return Array.from(units).sort();
}
// 학교급별 단원명 목록 가져오기 (초등학교만 지원)
function getUnitsBySchool(school) {
    if (school !== '초등학교') {
        return [];
    }
    const units = new Set();
...
function getUnitsByGrade(school, grade) {
    if (school !== '초등학교') {
        return [];
    }
    const units = new Set();
